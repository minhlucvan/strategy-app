...
class SplitArbStrategy(BaseStrategy):
    '''Split Arbitrage strategy'''
    _name = "SplitArb"
    desc = "Split Arbitrage strategy aiming to capture the price difference between the ex-split date and the split payment date. the study shown that for a bunch of stocks, the price of the stock will be raised before the ex-split date and drop after the split payment date, the strategy aims to capture the price difference between the two dates"
    param_def = [
        {
            "name": 'days_before',
            "type": "int",
    ...
    def run(self, calledby='add'):
        ...
        events_df = get_stocks_events(self.symbolsDate_dict, 'stockDividend')
