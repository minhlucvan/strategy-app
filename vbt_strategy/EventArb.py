...
class EventArbStrategy(BaseStrategy):
    '''Event Arbitrage strategy'''
    _name = "EventArb"
    desc = "Event Arbitrage strategy aiming to capture the price difference between the ex-event date and the event payment date. the study shown that for a bunch of stocks, the price of the stock will be raised before the ex-event date and drop after the event payment date, the strategy aims to capture the price difference between the two dates"
    param_def = [
        {
            "name": 'days_before',
            "type": "int",
    ...
    def run(self, calledby='add'):
        ...
        for stock in events_df.columns:
            for i in range(len(events_df)):
                if events_df[stock][i] not in selected_labels:
