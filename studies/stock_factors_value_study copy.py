...
def run(symbol_benchmark, symbolsDate_dict):
    ...
    pu.plot_multi_line(pe_ratio_df, title='PE/Industry PE')
    
    st.write("### Value Factor Strategy")
    
    entry_threshold = st.number_input('PE Ratio Entry Threshold', min_value=0.0, max_value=2.0, value=0.8)
    exit_threshold = st.number_input('PE Ratio Exit Threshold', min_value=0.0, max_value=2.0, value=1.2)
    
    # Run backtest
    pf, benchmark_returns = backtest_value_factor_v2(stocks_df, pe_ratio_df, benchmark_df, entry_threshold, exit_threshold)
    
    # Plot results
    fig, metrics, trades = plot_backtest_results(pf, benchmark_returns)
